...
interface DispositivoFormProps {
  isOpen: boolean;
  onClose: () => void;
  // Change onSubmit to accept DispositivoFormData and return a Promise
...
const DispositivoForm: React.FC<DispositivoFormProps> = ({
  ...
  const isEditing = !!initialData?.id; // Check for initialData AND its id
  ...
  // 3. useEffect to fetch models and set initial form data
  useEffect(() => {
    ...
    if (isOpen) {
      if (modelos.length === 0) { // Fetch models only if not already fetched
        fetchModelos();
      }
  ...
  }, [initialData, isEditing, isOpen, modelos.length]); // Added modelos.length to dependency array

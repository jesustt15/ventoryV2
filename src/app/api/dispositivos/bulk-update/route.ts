  ...
  await prisma.$transaction(async (tx) => {
    for (const row of rows) {
      ...
      if (!existing) {
        ...
        const nuevoDispositivo = await tx.dispositivo.create({
          data: {
            serial: row.serial,
            marca: row.marca,
            modeloId: modelo.id,
            tipo: row.tipo || modelo.tipo,
      ...
      if (row.marca !== null && row.marca !== undefined && row.marca !== existing.marca) dataToUpdate.marca = row.marca;
      if (row.tipo !== null && row.tipo !== undefined && row.tipo !== existing.tipo) dataToUpdate.tipo = row.tipo;
      ...
      if (row.imei !== null && row.imei !== undefined && row.imei !== existing.imei) dataToUpdate.imei = row.imei;

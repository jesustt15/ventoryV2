...
export async function POST(request: NextRequest) {
  try {
    ...
    titleCell.value = `Equipos Asignados - ${usuario}`;
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${usuario.replace(/\s+/g, '_')}_asignados_${new Date().toISOString().split("T")[0]}.xlsx"`,
      },
    });
  } catch (error: any) {
    console.error("[POST /api/usuarios/export-asignados]", error);
    return NextResponse.json(
